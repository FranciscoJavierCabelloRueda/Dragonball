...
$consulta = $conexion->prepare("DELETE FROM planetas WHERE id = ?");
...
if ($consulta->execute()) {
    http_response_code(200); // DELETE /api/planetas HTTP/1.1 200 OK 
    echo json_encode(['success' => true, 'message' => 'Planeta borrado de la BBDD correctamente']);
} else {
    http_response_code(500);
    echo json_encode(['error' => 'Error al borrar el planeta de la BBDD', 'details' => $consulta->error]);
}
